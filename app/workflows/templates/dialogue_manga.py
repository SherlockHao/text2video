...
10 Stages:
  ...
  6. video_prompts     — 4K 切图 + LLM 分镜视频指令 (16段)
...
{{
  ...
  "character_profiles": [
    {{
      ...
      "gender": "男/女",
      "age": "年龄描述",
      "appearance_prompt": "角色外貌的文生图提示词（严格遵守以下规则）",
      "voice_trait": "声音特征描述（如：低沉沙哑/清亮少女/沧桑中年）"
    }}
  ]
}}

【character_profiles.appearance_prompt 生成规则】
这是用于 T2I（文生图）模型的角色设定提示词，必须是**可直接绘制的视觉特征**。

必须包含（按优先级排序）：
1. 体型轮廓：如"身材高大魁梧"/"纤细瘦弱"/"微胖"
2. 发型与发色：如"黑色长直发"/"银白短发后梳"/"棕色卷发扎马尾"
3. 服装主色+款式：如"深灰色三件套西装"/"白色衬衫+黑色职业裙"/"粗布青衣"
4. 标志性道具/配饰（如有）：如"腰间铜铃"/"铂金袖扣"/"生锈铁棍"/"金丝眼镜"
5. 特殊标记（如有）：如"左眼疤痕"/"胸口黑色毒伤"/"手臂纹身"

禁止包含：
- 抽象感受："气场强大""充满压迫感""散发着XX气息"
- 嗅觉/听觉："木质香水味""声音低沉"
- 前后状态变化："前期空洞，后期凌厉"——只描述**初始登场状态**
- 动作描写："正在奔跑""挥舞武器"——只描述**静态外貌**
...
6. 风格统一为动漫/漫画风格
7. 每个 shot 必须标注 scene_group（场景分组标签），用英文标注场景位置（如 "Blacksmith Forge", "CEO Office"）。同一物理空间的分镜使用相同 scene_group。如果是回忆/闪回场景，在 scene_group 后加 "[FLASHBACK]"。
...
GRID_IMAGE_PROMPT_TEMPLATE = """You are generating a single 4x4 grid artwork (4 rows × 4 columns = 16 panels). The overall image MUST be in 16:9 WIDESCREEN aspect ratio (wider than tall). Each panel MUST also be 16:9 widescreen. Panels MUST be clearly separated by visible borders.
...
Generate the 4×4 grid image now. 16 clearly separated panels. MUST be 16:9 WIDESCREEN (wider than tall). No text anywhere."""

# ── 分镜视频 Prompt ──────────────────────────────────────────────

VIDEO_SEGMENTS_SYSTEM_PROMPT = """你是一个专业的漫剧视频分镜导演。

你将收到一个剧本单元的完整信息和16个分镜关键帧描述。你需要为每帧生成一个视频分镜段（共16段），并为每段输出详细的视频生成指令。

【规则】
1. 每段视频由对应的关键帧驱动（段N对应帧N）
2. 判断与上一段是否属于同一场景（same_scene=true），如果场景跳变则标记 same_scene=false
...
7. 【重要】video_prompt 规则：
   - is_dialogue=false 的段（纯动作/环境段），video_prompt 禁止出现"说话""开口""口型""讲述""台词""对话"等与人物发声有关的描写。只描述画面动态、镜头运动、表情变化。
   - is_dialogue=true 的段，video_prompt 也不要描述说话口型，只描述画面动态。台词内容由 dialogue 字段承载。
...
class DialogueMangaWorkflow(InteractiveOpsMixin, BaseWorkflow):
    ...
    def stage_storyboard_grids(self, ctx: WorkflowContext) -> StageResult:
        ...
        for ui, unit in enumerate(ctx.segments):
            ...
            story_context_parts = []
            # Translate emotion to English for Gemini
            _emotion_en = {
                "悬疑紧张": "suspenseful tension",
                "孤独迷离": "lonely and mysterious",
                "震撼共情": "shocking empathy",
                "甜宠轻松": "sweet and lighthearted",
                "热血燃": "passionate and thrilling",
            }
            emotion = unit.get("emotion_tone", "")
            story_context_parts.append(f"Emotional tone: {_emotion_en.get(emotion, emotion)}")
            story_context_parts.append(f"Story conflict: {unit.get('core_conflict', title)}")
    ...
    def stage_video_prompts(self, ctx: WorkflowContext) -> StageResult:
        ...
        for ui, unit in enumerate(ctx.segments):
            ...
            if len(existing_frames) >= 16:
                ...
                ctx.log(f"    ✓ 16 帧已切出 ({resize_target[0]}x{resize_target[1]})")
    ...
    def stage_video_gen(self, ctx: WorkflowContext) -> StageResult:
        ...
        for ui, unit in enumerate(ctx.segments):
            ...
            for seg in segments:
                ...
                use_sound = "off" if (is_dialogue or has_speech_hint) else "on"
    ...
    def stage_subtitle_burn(self, ctx: WorkflowContext) -> StageResult:
        ...
        if not font_path:
            ctx.log("  ⚠ 未找到 CJK 字体，跳过字幕压制。请安装 fonts-noto-cjk。")
            return StageResult(success=True, message="No CJK font found, subtitles skipped")
        ...
        for ui, unit in enumerate(ctx.segments):
            ...
            if not os.path.exists(bgm_path) or os.path.getsize(bgm_path) < 100:
                ...
                music_prompt = chat_with_system(
                    "你是影视配乐师。根据剧本信息生成一个 AI 音乐生成的英文 prompt，50-80词，只输出 prompt。要求：纯器乐配乐(instrumental only)，禁止人声/歌词/哼唱/合唱，只用乐器和音效。",
            ...
            if os.path.exists(bgm_path) and os.path.getsize(bgm_path) > 100:
                ...
                bgm_mean_db = -20  # fallback
                bgm_max_db = 0.0   # fallback
                for line in probe_cmd.stderr.split("\n"):
                    if "mean_volume" in line:
                        try:
                            bgm_mean_db = float(line.split("mean_volume:")[1].strip().replace(" dB", ""))
                        except (ValueError, IndexError):
                            pass
                    elif "max_volume" in line:
                        try:
                            bgm_max_db = float(line.split("max_volume:")[1].strip().replace(" dB", ""))
                        except (ValueError, IndexError):
                            pass

                adjust_db = bgm_target_db - bgm_mean_db
                if adjust_db > 0:
                    # 上调时防 clipping: 调整后 max 不能超过 -1dB
                    max_after = bgm_max_db + adjust_db
                    headroom = -1.0 - bgm_max_db  # 最多能加多少
                    if max_after > -1.0:
                        adjust_db = min(adjust_db, headroom)
                ...
                if result.returncode == 0:
                    ...
                    # BGM mix failed — try simpler mix: video + dialogue patch only (no BGM)
                    if has_patch:
                        ctx.log("    尝试仅混入补充对话...")
                        fallback_cmd = [
                            "ffmpeg", "-y",
                            "-i", concat_out, "-i", dialogue_patch_path,
                            "-filter_complex",
                            f"[0:a]volume=-20dB[vid];"
    ...
    # ================================================================
    # 覆写 mixin 中与 dialogue_manga 数据结构不兼容的方法
    # ================================================================

    def op_review_tts(self, output_dir: str) -> dict:
        """对话漫剧版 TTS 审查 — 按 unit/segment 结构返回。"""
        result = []
        for ui in range(1, 20):
            vp_path = os.path.join(output_dir, "grids", f"video_segments_u{ui}.json")
            if not os.path.exists(vp_path):
                break
            with open(vp_path) as f:
                segments = json.load(f).get("video_segments", [])
    ...
    def op_edit_storyboard(self, output_dir: str, segment: int,
        ...
        """对话漫剧版分镜编辑 — segment 参数对应 unit_number。"""
        ...
        allowed_fields = {"title", "core_conflict", "emotion_tone", "ending_hook"}
        if field not in allowed_fields:
            return {"error": f"Field '{field}' not editable. Allowed: {allowed_fields}"}
    ...
    def op_reroll_tts(self, output_dir: str, segment: int,
        ...
        """对话漫剧不使用 narration TTS reroll，请使用 op_reroll_dialogue_tts。"""
        return {"error": "Use 'reroll dialogue_tts' for dialogue_manga (specify --unit and --seg)"}

    def op_reroll_video(self, output_dir: str, seg: int, sub: int) -> dict:
        """对话漫剧不使用 sub_shot video reroll，请使用 op_reroll_video_segment。"""
        return {"error": "Use 'reroll video_segment' for dialogue_manga (specify --unit and --seg)"}
    ...
    def op_review_unit(self, output_dir: str, unit_number: int) -> dict:
        ...
        for seg in vp_data.get("video_segments", []):
            ...
            if os.path.isdir(frame_dir):
                ...
                frame_indices = [x for x in [sf, ef] if x is not None]
    ...
    def op_reroll_frame(self, output_dir: str, unit_number: int,
        ...
        # 切出 16 帧（智能检测分隔线 + resize 到统一 16:9）
